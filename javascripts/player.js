...
class PlayerCreator {
    ...
    bindEventListener () {
        ...
        // 可以播放的时候触发 （歌曲的基本信息都已经获取到了） 视频(video)准备开始播放时执行
    ...
    changeSongIndex (type) { // 'next' 'prev' number
        ...
        if ( typeof type === 'number' ) {
            ...
            if (this.loop_style === 0) {
                ...
                this.song_index = parseInt(Math.random()*this.musics.songs.length);
    ...
    changeSong (type) {
        ...
        // 记录切歌前的状态
        let _is_pause = this.audio.paused;  //如果为true表明歌曲暂停
        // 切歌之后更改视图显示
        this.renderSongStyle();
        // 如果切歌前是在播放，就继续播放
...
class Progress {
    ...
    renderBackAndPointer () {
        this.$back = $('<div class="back">');  //进度条背景宽度
        this.$pointer = $('<div class="pointer">'); //小点
